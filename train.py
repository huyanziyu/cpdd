...
from backbone import resnet50_fpn_backbone  # 导入创建骨干网络的函数
from network_files import MaskRCNN
...
def create_model(num_classes, load_pretrain_weights=True):
    ...
    if load_pretrain_weights:
        ...
        # 删除与类别预测器和 TwoMLPHead 相关的权重
        for k in list(weights_dict.keys()):
            if ("box_predictor" in k) or ("mask_fcn_logits" in k) or ("box_head.fc6" in k) or ("box_head.fc7" in k):
                del weights_dict[k]

        # 加载剩余的权重
        model.load_state_dict(weights_dict, strict=False)

        # 重新初始化 TwoMLPHead 中的 fc6 和 fc7 层
        in_features_fc6 = model.roi_heads.box_head.fc6.in_features
        model.roi_heads.box_head.fc6 = torch.nn.Linear(in_features_fc6, 1000)
        model.roi_heads.box_head.fc7 = torch.nn.Linear(1000, 1000)
...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for images, targets, stat_features in train_data_loader:
            images = [image.to(device) for image in images]
            targets = [{k: v.to(device) for k, v in t.items()} for t in targets]
            stat_features = torch.stack(stat_features).to(device)

            # 调用模型的 forward 方法
            loss_dict = model(images, targets, stat_features)
...
if __name__ == "__main__":
    ...
    # 训练数据集的根目录
    parser.add_argument('--data-path', default='D:\\pythondeeplearn\\tif', help='dataset')
    # 检测目标类别数(不包含背景)
    parser.add_argument('--num-classes', default=4, type=int, help='num_classes')
    # 训练数据集的根目录
    # parser.add_argument('--data-path', default=r'D:\pythondeeplearn/coco', help='dataset')
    # 检测目标类别数(不包含背景)
    # parser.add_argument('--num-classes', default=90, type=int, help='num_classes')
    # 文件保存地址
    parser.add_argument('--output-dir', default=r'D:\pythondeeplearn\tif', help='path where to save')

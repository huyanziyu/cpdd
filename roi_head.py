...
class RoIHeads(torch.nn.Module):
    ...
    def __init__(self,
        ...
        self.stat_feature_fc = nn.Linear(stat_feature_dim, 1024 - 1000)
    ...
    def forward(self,
        ...
        flattened_box_features = torch.flatten(box_features, start_dim=1)

        # 特征融合
        if stat_features is None or stat_features.nelement() == 0:
            # 如果为空，使用默认值（例如零向量）
            stat_features = torch.zeros((flattened_box_features.size(0), self.stat_feature_fc.in_features),
                                        device=flattened_box_features.device)

            # 转换统计特征
        transformed_stat_features = self.stat_feature_fc(stat_features)

        # 拼接特征
        combined_features = torch.cat((flattened_box_features, transformed_stat_features), dim=1)
        class_logits, box_regression = self.box_predictor(combined_features)
